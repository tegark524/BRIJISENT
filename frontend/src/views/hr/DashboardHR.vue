...
const fetchData = async () => {
  try {
    const resSum = await axios.get('http://127.0.0.1:8000/api/hr/dashboard-summary')
    summary.value = resSum.data.summary || { total_interns: 0, present_today: 0 }
    
    // Tarik data Realtime & Cek Libur
    const resRealtime = await axios.get('http://127.0.0.1:8000/api/hr/realtime-status')
    ...
    const resIntern = await axios.get('http://127.0.0.1:8000/api/users/intern')
    interns.value = resIntern.data.data || []

    const resHR = await axios.get('http://127.0.0.1:8000/api/users/hr')
    hrList.value = resHR.data.data || []
    
    // Tarik Semua History
    const resHistory = await axios.get('http://127.0.0.1:8000/api/hr/all-history')
    allHistoryData.value = resHistory.data.data || []
    
    const resSettings = await axios.get('http://127.0.0.1:8000/api/hr/settings')
...
const saveSettings = async () => {
  try {
    await axios.post('http://127.0.0.1:8000/api/hr/settings', attendanceSettings.value)
...
const addHoliday = async () => {
  ...
  try {
    await axios.post('http://127.0.0.1:8000/api/hr/holidays', newHoliday.value)
...
const deleteHoliday = async (id) => {
  try {
    await axios.delete(`http://127.0.0.1:8000/api/hr/holidays/${id}`)
    fetchData()
  } catch (e) {
...
const handleSave = async () => {
  ...
  try {
    if (isEditMode.value) await axios.put(`http://127.0.0.1:8000/api/users/${form.value.id}`, form.value);
    else await axios.post(`http://127.0.0.1:8000/api/users`, { ...form.value, role: currentRole });
...
const confirmDelete = (id) => {
  ...
  .then(async (result) => {
    if (result.isConfirmed) { await axios.delete(`http://127.0.0.1:8000/api/users/${id}`); fetchData(); Swal.fire('Deleted', 'Data dihapus', 'success'); }
  })
}
